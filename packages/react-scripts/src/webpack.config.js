...
module.exports = (isEnvProduction = process.env.NODE_ENV === 'production') => {
  const publicUrl = isEnvProduction ? publicPath.slice(0, -1) : !isEnvProduction && '';
  ...
  const babelLoaderConfig = {
    ...
    options: {
      ...
      presets: [
        ...
        ["@babel/preset-react", { development: !isEnvProduction, useBuiltIns: true, }]
      ],
      plugins: [
  ...
  return {
    ...
    mode: isEnvProduction ? 'production' : !isEnvProduction && 'development',
    bail: isEnvProduction,
    devtool: isEnvProduction
      ? shouldUseSourceMap
        ? 'source-map'
        : false
    : !isEnvProduction && 'eval-source-map',
    node: {
      module: 'empty',
      dgram: 'empty',
      dns: 'mock',
